...
n_classes = len(italian_book_dataset.samples)
...
class_to_ix = dict()
ix_to_class = dict()
for index, (file_name, class_name, title) in enumerate(italian_book_dataset.samples):
    doc_name = class_name + u"_" + title
    class_to_ix[doc_name] = index
    ix_to_class[index] = doc_name
# end for
...
for k in range(10):
    ...
    for i, data in enumerate(italian_loader_train):
        ...
        # Doc name
        doc_name = labels[0] + u"_" + title[0]

        # Input dim
        inputs = inputs.view(1, -1, transformer_output_dim)

        # Log
        print(u"\tTraining with {} ({}), {} tokens".format(doc_name, class_to_ix[doc_name], inputs.size(1)))

        # Time labels
        time_labels = functions.to_timelabels(inputs.size(1), class_to_ix[doc_name], n_classes)
    ...
    print(training_oov[k])
    # Finalize training
    print(u"training")
    esn.finalize()
    print(u"end")
    print(esn.output.w_out)
    exit()
    # Counters
    successes = 0.0
    count = 0.0
    ...
    for i, data in enumerate(italian_loader_test):
        ...
        if args.cuda: inputs, labels = inputs.cuda(), labels.cuda()
